...
**Run this after edits are made since it makes the runs longer**
/*
...
/*by occ degfield, sort: gen frequency = _N
keep if frequency > 1
by occ degfield: keep if _n == 1
by occ degfield(frequency degfield), sort: gen _j = _n
reshape wide degfield frequency, i(occ) j(_j)
*/



*collapse so that there is one row per occupational code
collapse (median)yrsed (mean)mode1_deg, by(occ)
