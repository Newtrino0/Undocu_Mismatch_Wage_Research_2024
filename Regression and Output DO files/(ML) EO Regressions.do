...
global covars i.age hisp male gov_worker immig_by_ten nonfluent yrsed i.degfield_broader metropolitan
...
esttab est* using dTable_status_ml.tex, replace label main(mean) aux(sd) title("ACS U.S. born workers and Undocumented immigrants Summary Statistics \label{tab:sum}") unstack mlabels("Foreign-born" "Undocumented (Logical edits)" "Undocumented (KNN)" "Undocumented (RF)") note("Note: Log wage is adjusted for inflation with CPI values starting January 2009, every year in January until January 2019.")
...
eststo: estpost tabstat age married nonfluent  household_size poverty asian black white other_race bpl_asia central_latino spanish_hispanic_latino  employed years_us yrsed undocu_likely if yrsed>=16, statistics(mean sd) columns(statistics)
eststo: estpost tabstat age married nonfluent  household_size poverty asian black white other_race bpl_asia central_latino spanish_hispanic_latino  employed years_us yrsed undocu_likely if yrsed>=16 & undocu_likely==1, statistics(mean sd) columns(statistics)   
eststo: estpost tabstat age married nonfluent  household_size poverty asian black white other_race bpl_asia central_latino spanish_hispanic_latino  employed years_us yrsed undocu_likely if yrsed>=16 & sipp_knn==1, statistics(mean sd) columns(statistics) 
eststo: estpost tabstat age married nonfluent  household_size poverty asian black white other_race bpl_asia central_latino spanish_hispanic_latino  employed years_us yrsed undocu_likely if yrsed>=16 & sipp_rf==1, statistics(mean sd) columns(statistics) 
...
**Start writing, add IPC table last
**RERUN TABLES 3-4 with new undocu measures
**Conduct literature review (Van Hook, Stimpson, )
...
use "(ML) Pre Regression sample",clear
***Logical edits V. mismatch model***
reg vmismatched hundermatched hovermatched elig inclusive exclusive  $covars  i.statefip##i.year  [pweight=perwt] if year >= 2013, r cl(statefip)
estadd ysumm
eststo
***KNN V. mismatch model***
reg vmismatched hundermatched hovermatched elig_knn inclusive exclusive  $covars  i.statefip##i.year  [pweight=perwt] if year >= 2013, r cl(statefip)
estadd ysumm
eststo

***RF V. mismatch model***
reg vmismatched hundermatched hovermatched elig_rf inclusive exclusive $covars  i.statefip##i.year  [pweight=perwt] if year >= 2013, r cl(statefip)
estadd ysumm
eststo
...
use "(ML) Pre Regression sample",clear
***Logical edits H. undermatch model***
reg hundermatched vmismatched elig inclusive exclusive  $covars  i.statefip##i.year  [pweight=perwt] if year >= 2013, r cl(statefip)
estadd ysumm
eststo
***KNN H. undermatch model***
reg hundermatched vmismatched elig_knn inclusive exclusive  $covars  i.statefip##i.year  [pweight=perwt] if year >= 2013, r cl(statefip)
estadd ysumm
eststo
***RF H. undermatch model***
reg hundermatched vmismatched elig_rf inclusive exclusive  $covars  i.statefip##i.year  [pweight=perwt] if year >= 2013, r cl(statefip)
estadd ysumm
eststo
...
use "(ML) Pre Regression sample",clear
***Logical edits column***
reg ln_adj vmismatched hundermatched hovermatched elig inclusive exclusive  $covars   i.statefip##i.year [pweight=perwt] if year >= 2013, r cl(statefip)
estadd ysumm 
eststo
**KNN column***
reg ln_adj vmismatched hundermatched hovermatched elig_knn inclusive exclusive  $covars   i.statefip##i.year [pweight=perwt] if year >= 2013, r cl(statefip)
estadd ysumm
eststo
**RF column***
reg ln_adj vmismatched hundermatched hovermatched elig_rf inclusive exclusive  $covars   i.statefip##i.year [pweight=perwt] if year >= 2013, r cl(statefip)
estadd ysumm
eststo
