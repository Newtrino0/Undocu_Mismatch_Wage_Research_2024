...
features <- c("age", "fem", "married", "cit_spouse", "medicaid", "nonfluent", "spanish_hispanic_latino", "central_latino", "bpl_asia", "household_size", "poverty", "asian", "black", "white", "other_race", "employed", "years_us", "years_us_missing", "yrsed")
...
train_index_gbm <- createDataPartition(sipp08_2$undocu_likely, p = 0.7, list = FALSE)
# Master Train and Test sets
train_gbm <- sipp08_2[train_index_gbm, ]
test_gbm <- sipp08_2[-train_index_gbm, ]


# The college models train ONLY on the college grads from the master training set
train_college <- train_gbm %>% filter(college == 1)

# ALL models are evaluated on the exact same holdout college grads
test_college <- test_gbm %>% filter(college == 1)
...
gbm_model <- gbm(undocu_likely ~ age + fem + married + cit_spouse + medicaid + nonfluent + spanish_hispanic_latino + central_latino + bpl_asia + household_size + poverty + asian + black + white + other_race + employed  + years_us + years_us_missing + yrsed, data = train_gbm, 
                 verbose = FALSE,
                 cv.folds = 10,
                 n.trees = 3500,
                 shrinkage = 0.005,
...
set.seed(1)
## Data partitioning for gbm library
sipp08_2_college$undocu_likely <- ifelse(sipp08_2_college$undocu_likely %in% c("X1", "1", 1), 1, 0)



train_index_college <- createDataPartition(sipp08_2_college$undocu_likely, p = 0.7, list = FALSE)
train_college <- sipp08_2_college[train_index_college, ]
test_college <- sipp08_2_college[-train_index_college, ]

college_gbm_model <- gbm(undocu_likely ~ age + fem + married + cit_spouse + medicaid + nonfluent + spanish_hispanic_latino + central_latino + bpl_asia + household_size + poverty + asian + black + white + other_race + employed  + years_us + years_us_missing + yrsed, data = train_college, 
                 verbose = FALSE,
                 cv.folds = 10,
                 n.trees = 3500,
                 shrinkage = 0.005, 
...
setwd(figures_path)
## Feature importance
college_gbm_feature_importance <- vip(college_gbm_model, num_features = 22, bar = FALSE)
...
trapped_count <- sum(ACS$gbm_undocu_p >= 0.53816669 & ACS$gbm_undocu_p < 0.5605042, na.rm = TRUE)
