...
train_index_svm <- createDataPartition(all_data$sample$undocu_likely, p = 0.7, list = FALSE)
train_svm <- all_data$sample[train_index_svm, ]
test_svm <- all_data$sample[-train_index_svm, ]
...
svm_linear <- train(
  ...
  tuneGrid = expand.grid(C = c(1, 10))  # Cost parameter
)

# Polynomial SVM (svmPoly)
svm_poly <- train(
  undocu_likely ~ age + fem + married + cit_spouse + medicaid + nonfluent + spanish_hispanic_latino + central_latino + bpl_asia + household_size + poverty + asian + black + white + other_race + employed + years_us + yrsed, 
  data = train_svm,
  method = "svmPoly",
  ...
  tuneGrid = expand.grid(
    ...
    scale = c(0.01, 0.1, 1),        # Scale parameter
    C = c(1, 10)               # Cost parameter
  )
)
...
svm_rbf <- train(
  ...
  metric = "ROC",
  tuneGrid = expand.grid(
    sigma = c(0.001, 0.01, 0.1, 1), # Sigma (gamma) parameter
    C = c(1, 10)          # Cost parameter
  )
)
