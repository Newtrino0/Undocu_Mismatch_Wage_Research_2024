...
# GBM first
```{r Gradient-Boosting}
gbmFit1 <- train( undocu_likely ~ age + fem + married + cit_spouse + medicaid + nonfluent + spanish_hispanic_latino + central_latino + bpl_asia + household_size + poverty + asian + black + white + other_race + employed + years_us + yrsed, data = training, 
                 method = "gbm", 
                 trControl = upcontrol,
                 verbose = FALSE)
gbmFit1
...
```{r Loading ACS data}
library(readr)
setwd("C:/Users/ecran/Documents/Undocu Research Data")
