...
sipp08_2_knn <- all_data$knn
sipp08_2_sample <- all_data$rf
sipp08_2_noncit <- all_data$noncit
sipp08_2_central_latino <- all_data$central_latino
...
train_index_gbm <- createDataPartition(all_data$sample_num$undocu_likely_num, p = 0.7, list = FALSE)
train_gbm <- all_data$sample_num[train_index_gbm, ]
test_gbm <- all_data$sample_num[-train_index_gbm, ]
                   


gbm_model <- gbm( undocu_likely_num ~ age + fem + married + cit_spouse + medicaid + nonfluent + spanish_hispanic_latino + central_latino + bpl_asia + household_size + poverty + asian + black + white + other_race + employed + years_us + yrsed, data = train_gbm, 
                 verbose = FALSE,
                 cv.folds = 10,
                 n.trees = 1766,
                 shrinkage = 0.005, #Optimi
                 interaction.depth = 6)
gbm_model


# Make predictions on test set
boost.preds2 = predict(gbm_model, newdata = test_gbm, type = "response", n.trees = 1766)  # Get probabilities for class "1"
# Create prediction object for ROCR
boost.prediction = prediction(boost.preds2, test_gbm$undocu_likely_num)
# Get raw predictions (class labels)
boost.predict = predict(gbm_model, newdata = test_gbm, n.trees=1766)  # Class predictions

boost.perf = performance(boost.prediction,"tpr","fpr")   # ROC curve
boost.perf2 = performance(boost.prediction,"prec","rec") # Precision-Recall curve
boost.auc <- performance(boost.prediction,"auc")@y.values[[1]] # AUC value

boost_auc_pr <- trapz(
  as.vector(boost.perf2@x.values)[[1]][-1], 
  as.vector(boost.perf2@y.values)[[1]][-1]
)

gbm_performance <- boost.perf2
plot(boost.perf, main = "ROC Curve", col = "blue", lwd = 2) #PLOT EACH ON A SINGLE GRAPH FOR ROC
abline(a = 0, b = 1, lty = 2, col = "red")
plot(boost.perf2) # plot Precision-Recall curve
...
predicted_class <- ifelse(boost.preds2 > 0.5, 1, 0)
table(Predicted = predicted_class, Actual = test_gbm$undocu_likely_num)
...
# --- Create groups (deciles of boost_preds, reversed so 1 = most likely) ---
data <- data %>%
  mutate(groups = ntile(boost_preds, 10),
         groups = 11 - groups)

# --- Variables ---
data <- data %>%
  group_by(groups) %>%
  mutate(lowest_pred = min(boost_preds)) %>%
  ungroup() %>%
  mutate(
    female     = as.integer(sex == 0),
    teen       = as.integer(age < 20),
    youngadult = as.integer(age >= 20 & age < 30),
    one        = 1,
    hisp       = as.integer(hispanic == 1),
    bsh        = as.integer(black == 1 | hispanic == 1),
    white      = as.integer(race == 1),
    hsg        = as.integer(hsl == 1 & hsd == 0)
  )

# --- Variables of interest ---
full_list <- c("teen", "youngadult", "hsd", "hsg", "female", "white", "bsh")

# --- Compute stats by decile ---
summary_list <- list()

for (vv in c("one", "lowest_pred", full_list)) {
  if (vv == "one") {
    # count of obs per group
    temp <- data %>%
      group_by(groups) %>%
      summarise(value = n(), .groups = "drop")
    
  } else if (vv == "lowest_pred") {
    # min of boost_preds per group
    temp <- data %>%
      group_by(groups) %>%
      summarise(value = min(boost_preds), .groups = "drop")
    
  } else {
    # weighted mean of variable
    temp <- data %>%
      group_by(groups) %>%
      summarise(value = weighted.mean(.data[[vv]], w = earnwt, na.rm = TRUE), .groups = "drop")
  }
  
  summary_list[[vv]] <- temp$value
}

# --- Build table ---
# Turn into data frame (rows = variables, cols = deciles)
results_df <- do.call(rbind, summary_list)
rownames(results_df) <- names(summary_list)
colnames(results_df) <- paste0("Decile_", 1:10)
...
modelsummary(
  list(results_df), 
  output = paste0(Sys.getenv("tables"), "table_1.tex"),
  escape = FALSE # allow LaTeX in labels if needed
)
...
```{r Prep ACS .dta data}
# Convert labeled variables to character first, then process
ACS <- ACS %>%
  mutate(
    # Convert any labeled variables to character
    hisp = as.numeric(as.character(hisp)),
    spanish = as.numeric(as.character(spanish)),
    race = as.character(race),
    bpld = as.character(bpld),
    empstat = as.numeric(as.character(empstat)),
    poverty = as.numeric(as.character(poverty)),
    married = as.numeric(as.character(married)),
    cit_spouse = as.numeric(as.character(cit_spouse)),
    nonfluent = as.numeric(as.character(nonfluent)),
    bpl_asia = as.numeric(as.character(bpl_asia)),
    asian = as.numeric(as.character(asian)),
    fem = as.numeric(as.character(fem)),
    numprec = as.numeric(as.character(numprec)),
    yrsusa1 = as.character(yrsusa1),  # Keep as character for string matching
    hinscare = as.character(hinscare),
    hinscaid = as.character(hinscaid),
    undocu = as.numeric(as.character(undocu)),
    bpl_usa = as.numeric(as.character(bpl_usa)),
    age = as.numeric(as.character(age)),
    yrsed = as.numeric(as.character(yrsed))
  )
...
ACS_sipp <- ACS %>%
  mutate(
    ...
    # Handle case-insensitive string matching for bpld
    central_latino = as.factor(ifelse(
      (hisp == 1) & (tolower(bpld) %in% c("belize/british honduras", "costa rica", 
                                         "el salvador", "guatemala", "honduras", 
                                         "nicaragua", "panama", "mexico")), 1, 0)),
    
    # Handle case-insensitive race matching
    black = as.factor(ifelse(tolower(race) == "black/african america" | 
                            tolower(race) == "black/african american", 1, 0)),
    white = as.factor(ifelse(tolower(race) == "white", 1, 0)),
    ...
    # Handle years_us with proper string matching
    years_us = case_when(
      tolower(yrsusa1) == "n/a or less than one year" & bpl_usa == 1 ~ NA_real_,
      tolower(yrsusa1) == "n/a or less than one year" & bpl_usa == 0 ~ 0,
      TRUE ~ as.numeric(yrsusa1)
    ),
    
    # Handle case-insensitive insurance matching
    medicare = as.factor(ifelse(tolower(hinscare) == "yes", 1, 0)),
    medicaid = as.factor(ifelse(tolower(hinscaid) == "has insurance through medicaid", 1, 0))
  )

# Fix other_race calculation (use ACS_sipp instead of ACS)
ACS_sipp$other_race <- as.factor(ifelse(
  as.numeric(as.character(ACS_sipp$black)) != 1 & 
  as.numeric(as.character(ACS_sipp$white)) != 1 & 
  as.numeric(as.character(ACS_sipp$asian)) != 1, 1, 0))
...
boost_pr_data <- data.frame(
  recall = unlist(boost.perf2@x.values), 
  precision = unlist(boost.perf2@y.values)
)

# Interpolate other models to match boosted trees recall values
rf_pr_data <- data.frame(
  recall = approx(unlist(rf.perf2@x.values), unlist(rf.perf2@y.values), 
                 xout = boost_pr_data$recall)$x,
  precision = approx(unlist(rf.perf2@x.values), unlist(rf.perf2@y.values), 
                    xout = boost_pr_data$recall)$y
)

logistic_pr_data <- data.frame(
  recall = approx(unlist(logistic.perf2@x.values), unlist(logistic.perf2@y.values), 
                 xout = boost_pr_data$recall)$x,
  precision = approx(unlist(logistic.perf2@x.values), unlist(logistic.perf2@y.values), 
                    xout = boost_pr_data$recall)$y
)

knn_pr_data <- data.frame(
  recall = approx(unlist(knn.perf2@x.values), unlist(knn.perf2@y.values), 
                 xout = boost_pr_data$recall)$x,
  precision = approx(unlist(knn.perf2@x.values), unlist(knn.perf2@y.values), 
                    xout = boost_pr_data$recall)$y
)
...
legend(x = 0.5, y = 1, 
       ...
       lwd = c(3, 3, 3, 3, 3), 
       lty = c(0, 1, 2, 3, 4),
       legend = c("Model:", "Boosted Trees", "Random Forest", "Logistic", "KNN"),
...
ACS_sipp_na$gbm_undocu_p <- predict(gbm_model, ACS_sipp_na)
