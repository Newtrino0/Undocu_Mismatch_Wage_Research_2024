...
* rcuown57: Person number of the owner of the SS coverage US Citizenship Status of Respondent. 
...
sipp08_2 <- SIPP_2008_Wave_2 %>%
  mutate(
    ...
    race = case_when(
      erace=="Asian only" ~ "Asian",
      erace=="Black only" ~ "Black",
      erace=="White only" ~ "White",
      erace=="Residual" ~ "Other",
      TRUE ~ "Unknown"
    ),
    employed = case_when(
      rmesr=="With a job at least 1 but not all" | rmesr=="With a job entire month, absent" | rmesr=="With a job entire month, worked" ~  "Yes",
      rmesr=="Not in Universe" ~ "Not in Universe",
      TRUE ~ "No"
    ),
...
sipp08_2$year_us <- replace(sipp08_2$years_us, sipp08_2$years_us == 2008 | sipp08_2$years_us == 2009, NA)
sipp08_2$tage <- replace(sipp08_2$tage, sipp08_2$tage == "Less than 1 full year old", 0)
sipp08_2$age <- as.numeric(sipp08_2$tage)
sipp08_2$undocu_likely <- replace(sipp08_2$undocu_likely, )
...
train_index <- createDataPartition(sipp08_2$undocu_likely, p = 0.7, list = FALSE)
train_data <- sipp08_2[train_index, ]
test_data <- sipp08_2[-train_index, ]

undocu_model <- glm(undocu_likely ~ age + college + hs_only + married + english_difficult + spanish_hispanic_latino + medicaid + household_size + poverty + race + employed + citizen + years_us, data = train_data, family = binomial)

summary(undocu_model)
...
pred_probs <- predict(undocu_model, newdata = test_data, type = "response")
...
#ML statistics
accuracy <- logistic_matrix$overall[['Accuracy']]
sensitivity <- logistic_matrix$byClass[['Sensitivity']]
specificity <- logistic_matrix$byClass[['Specificity']]
ppv <- logistic_matrix$byClass[['Pos Pred Value']]
```