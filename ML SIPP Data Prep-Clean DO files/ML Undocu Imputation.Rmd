...
sipp08_2_logistic <- sipp08_2[sipp08_2$citizen == 0 & sipp08_2$age>=20, c("undocu_likely", "undocu_logical", "age", "college", "hs_only", "married", "english_difficult", "spanish_hispanic_latino", "medicaid", "household_size", "poverty", "asian", "black", "white", "other_race", "employed", "years_us","yrsed")]
sipp08_2_knn <- sipp08_2[sipp08_2$citizen == 0 & sipp08_2$age>=20, c("undocu_likely", "age", "college", "hs_only", "married", "english_difficult", "spanish_hispanic_latino", "medicaid", "household_size", "poverty", "asian", "black", "white", "other_race", "employed", "years_us","yrsed")]
...
plot(cooks.distance(logistic_model), type = "h", main = "Cook’s Distance") 

ggplot(sipp08_2_logistic, aes(x = age, y = household_size)) +
...
logistic_model <- glm(undocu_likely ~ age + college + hs_only + married + english_difficult + spanish_hispanic_latino + medicaid + household_size + poverty + asian + black + white + other_race + employed + years_us + yrsed, data = train_data, family = binomial)
...
logistic_kmodel <- train(undocu_likely ~ age + college + hs_only + married + english_difficult + spanish_hispanic_latino + medicaid + household_size + poverty + asian + black + white + other_race + employed + years_us + yrsed, sipp08_2_klogistic,
               method = "glm",
               trControl = control)

p_klogistic <- predict(logistic_kmodel, sipp08_2_klogistic)

# Generate confusion matrix
klogistic_matrix <- confusionMatrix(p_klogistic, sipp08_2_klogistic$undocu_likely, positive = "X1")
...
View(sipp08_2_knn)
train_scaled <- scale(train_knn[, 2:17])
test_scaled <- scale(test_knn[, 2:17])
...
# Generate ROC curve
roc_curve <- roc(test_data$undocu_likely, predictions)
...
# ML statistics
accuracy <- c(accuracy_logical, logistic_matrix$overall[['Accuracy']], klogistic_matrix$overall[['Accuracy']], knn_matrix$overall[['Accuracy']])

sensitivity <- c(sensitivity_logical, logistic_matrix$byClass[['Sensitivity']], klogistic_matrix$byClass[['Sensitivity']], knn_matrix$byClass[['Sensitivity']])

specificity <- c(specificty_logical, logistic_matrix$byClass[['Specificity']], klogistic_matrix$byClass[['Specificity']], knn_matrix$byClass[['Specificity']])

ppv <- c(ppv_logical, logistic_matrix$byClass[['Pos Pred Value']], klogistic_matrix$byClass[['Pos Pred Value']], knn_matrix$byClass[['Pos Pred Value']])


ml_table <- data.frame(sensitivity, specificity, ppv, accuracy)
ml_comparison <- as.data.frame(t(ml_table))
colnames(ml_comparison) <- c("Logical edits", "Logistic Classification", "K-folds Logistic", "KNN Classifier")
...
accuracy_values <- sapply(k_values, function(k) {
  ...
  1 - mean(knn_model != train_knn$undocu_likely)
})
