...
use "(Undocu)EO_Step_2.dta", clear
...
egen undocu_occ_count=count(occ) if undocu==1, by(occ)
egen undocu_deg_count=count(degfield) if undocu==1, by(degfield)
...
save "(Undocu)EO_Final_Sample", replace